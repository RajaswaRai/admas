...
if (!empty($_SESSION['nik'])) {
    ...
    $sql_pengaduan = "SELECT * FROM pengaduan pa INNER JOIN masyarakat ma ON pa.nik = ma.nik WHERE `status`='0' ORDER BY id_pengaduan DESC";
    $exc_pengaduan = mysqli_query($koneksi, $sql_pengaduan);
    $jml_pengaduan = mysqli_num_rows($exc_pengaduan);

    header("content-type: application/vnd-ms-excel");
    header("Content-Disposition: attachment; filename=report_belum.xls");
?>

    <h4>Tabel Pengaduan Belum Diproses</h4>
